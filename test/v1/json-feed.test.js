  ...
  it('it should an error when missing query string', (done) => {
    chai.request(server)
      ...
      .end((err, res) => {
         ...
         res.body.request.should.have.property('url').eql(`${HOST}/api/v1/jsonfeed`);
         res.body.should.have.property('apiVersion').eql("1");
         done();
      });
  }).timeout(10000);
  ...
  it('it should an error when the value of query string is not a URL', (done) => {
    chai.request(server)
      ...
      .end((err, res) => {
         ...
         res.body.request.should.be.a('object');
         res.body.request.should.have.property('method').eql("GET");
         res.body.request.should.have.property('url').eql(`${HOST}/api/v1/jsonfeed?url=abc`);
         res.body.should.have.property('apiVersion').eql("1");
         done();
      });
  }).timeout(10000);
  ...
  it('it should an error when the url is not a RSS feed URL', (done) => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
         ...
         res.body.request.should.be.a('object');
         res.body.request.should.have.property('method').eql("GET");
         res.body.request.should.have.property('url').eql(`${HOST}/api/v1/jsonfeed?url=${feedUrl}`);
         res.body.should.have.property('apiVersion').eql("1");
         done();
      });
  }).timeout(10000);
